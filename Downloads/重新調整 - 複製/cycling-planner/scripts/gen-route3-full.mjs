...
const WAYPOINTS = [
  { id: "wp3-khh", name: "高雄車站", lat: 22.6394, lng: 120.3024 },
  { id: "wp3-pif", name: "屏東車站", lat: 22.6698, lng: 120.4862 },
  { id: "wp3-ttt", name: "台東車站", lat: 22.7554, lng: 121.1436 },
];
...
const TARGETS = {
  ...
  convenience: 6,
  restaurantCheap: 6,
  restaurantExpensive: 6,
};

const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

function toBudgetTier(pricePerNight) {
  if (pricePerNight <= 1800) return "low";
  if (pricePerNight <= 4200) return "mid";
  return "high";
}
function hotelPrice(priceLevel, subtype) {
  if (subtype === "hostel") {
    const map = { 1: 900, 2: 1400, 3: 2000, 4: 2800 };
    return map[priceLevel] ?? 1400;
  }
  const map = { 1: 2800, 2: 3500, 3: 5000, 4: 7600 };
  return map[priceLevel] ?? 3500;
}
function restaurantPrice(priceLevel) {
  const map = { 1: 180, 2: 260, 3: 420, 4: 650 };
  return map[priceLevel] ?? 260;
}
function restaurantTier(priceLevel) {
  if (priceLevel === 3 || priceLevel === 4) return "expensive";
  return "cheap";
}
function pickUnique(candidates, count, seen) {
  const out = [];
...
async function nearbySearch({ lat, lng, radius, type, keyword = "", pages = 2 }) {
  ...
  for (let i = 0; i < pages; i++) {
    ...
    if (pageToken) {
      await sleep(2200);
      url = `https://maps.googleapis.com/maps/api/place/nearbysearch/json?pagetoken=${encodeURIComponent(pageToken)}&key=${API_KEY}`;
    } else {
...
async function placeDetails(placeId) {
  const fields = "name,geometry/location,price_level,formatted_address,types";
...
async function buildForWaypoint(wp) {
  ...
  if (hostelPicked.length < TARGETS.hostel) {
    const need = TARGETS.hostel - hostelPicked.length;
    const extra = pickUnique(lodgingCandidates, need, seenHostels);
    hostelPicked.push(...extra);
  }
  ...
  accommodations.push(
    ...(await enrichMany(hotelPicked, TARGETS.hotel, (d, c) => {
      const price = hotelPrice(d.price_level, "hotel");
      return { id: `acc-${wp.id}-${c.place_id}`, waypoint_id: wp.id, name: d.name || c.name, subType: "hotel", budget_tier: toBudgetTier(price), price_per_night: price, lat: d.geometry.location.lat, lng: d.geometry.location.lng, place_id: c.place_id, address: d.formatted_address || "" };
    }))
  );
  accommodations.push(
    ...(await enrichMany(hostelPicked, TARGETS.hostel, (d, c) => {
      const price = hotelPrice(d.price_level, "hostel");
      return { id: `acc-${wp.id}-${c.place_id}`, waypoint_id: wp.id, name: d.name || c.name, subType: "hostel", budget_tier: toBudgetTier(price), price_per_night: price, lat: d.geometry.location.lat, lng: d.geometry.location.lng, place_id: c.place_id, address: d.formatted_address || "" };
    }))
  );

  const restaurants = [];
  restaurants.push(
    ...(await enrichMany(convPicked, TARGETS.convenience, (d, c) => ({
      id: `res-${wp.id}-${c.place_id}`, waypoint_id: wp.id, name: d.name || c.name, diet_type: "convenience-store", price_tier: "standard", estimated_cost: 180, lat: d.geometry.location.lat, lng: d.geometry.location.lng, place_id: c.place_id, address: d.formatted_address || "",
    })))
  );
...
async function main() {
  const output = {
    Routes: [{ id: "south-corridor-3", name: "南部走廊（高雄→屏東→台東）", type: "south-corridor", recommended_days: 3, supported_start_stations: WAYPOINTS.map((w) => w.name) }],
    Waypoints: WAYPOINTS,
    Accommodations: [],
    Restaurants: [],
  };
  for (const wp of WAYPOINTS) {
    const block = await buildForWaypoint(wp);
    output.Accommodations.push(...block.accommodations);
    output.Restaurants.push(...block.restaurants);
  }
